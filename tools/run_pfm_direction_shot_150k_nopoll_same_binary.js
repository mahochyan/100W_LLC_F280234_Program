...
session.memory.loadProgram("D:\\CCS21_workspace\\Codex_Project\\evidence\\stage5a_strict_ab\\LLC_STAGE5A_2D1F2BA0.out");
...
wv("g_pfm_direction_test_mode",1);   // 150K
...
print("test_mode=1 run_id=0x250C5A15");
...
session.target.runAsynch();
java.lang.Thread.sleep(1000);   // NO target access during run; firmware self-schedules OST
...
print("slope_raw_per_ms_cycle = " + ((eraw-sraw)/0.30000).toFixed(3));
...
var pass = (res==8 && wc==wt && wt==45 && tbprd==399 &&
            fault==0 && hard==0 && stale==0 && pwmF==0 && ostF==1 && socaA==0 && ovfA==0);
print(pass ? "*** PFM_DIRECTION_150K_NOPOLL_SAME_BINARY_PASS ***" : "*** 150K_SAME_BINARY_GATE_NOT_MET — REVIEW ***");
session.target.disconnect();
print("DONE");

// W2 candidate4: on-target no-energy proof of the pre-handoff energy shaping.
// The no-energy synthetic VOUT is held at 1244 raw (>=10V target), so the
// SoftStart path must enter SOFTSTART_PRE_HANDOFF_BRAKE, settle with zero
// slope, and transfer to PI at the actual brake frequency (155 kHz initial).
...
var failures=0;
print("=== SOL W2 CANDIDATE4 PRE_BRAKE NOENERGY ===");
...
if(failures){ print("SOL_W2_CANDIDATE4_PRE_BRAKE_NOENERGY_PASS=FALSE"); try{session.terminate();}catch(e){} throw("PRE_GATE"); }
...
var pbEntry=rw("g_pre_brake_entry_raw_frozen");
var pbExit=rw("g_pre_brake_exit_raw_frozen");
var pbFreq=rv32u("g_pre_brake_freq_hz");
var pbPeriod=386;
...
var ssState=rw("g_softstart_state");
var ssFinalCycles=rw("g_softstart_final_cycles");
var ssResult=rw("g_softstart_result");

print("OBS transferred="+transferred+" tbprd="+tbprd+" cmpa="+cmpa+" cmpb="+cmpb+
      " db="+dbred+"/"+dbfed+" socaprd="+socaprd+" ss_state="+ssState+
      " ss_final_cycles="+ssFinalCycles+" ss_result="+ssResult);
...
print("OBS pre_brake entry="+pbEntry+" exit="+pbExit+" freq="+pbFreq+" period="+pbPeriod+
      " cycles="+pbCycles+" settle="+pbSettle+" max_dv="+pbMaxDv+
      " ready="+pbReady+" abort="+pbAbort);
...
check("PRE_BRAKE_WAS_USED",pbEntry>0 && pbExit>0 && pbReady==1 && pbCycles>0);
check("PRE_BRAKE_NO_ABORT",pbAbort==0);
check("PRE_BRAKE_PWM_STATE",tbprd==pbPeriod && cmpa==((pbPeriod+1)>>1) && dbred==36 && dbfed==36);
check("PRE_BRAKE_INITIAL_FREQ",pbFreq==155000 && pbPeriod==386);
check("CLOSED_LOOP_ET3_RESTORED",socaprd==3);
check("PRE_BRAKE_CONTROL_STATE",Math.abs(freq-155000)<=500 && Math.abs(shadow-155000)<=500);
check("PRE_BRAKE_INTEGRAL_STATE",Math.abs(integral+20480000)<=250000);
check("FIRST_PI_BUMPLESS",rw("g_stage6_first_pi_observed")==1 && Math.abs(firstfreq-155000)<=500);
...
print("SOL_W2_CANDIDATE4_PRE_BRAKE_NOENERGY_PASS="+(failures==0?"TRUE":"FALSE"));

// STAGE6_40US_SPLIT_PIPELINE — G9: single 1ms LIGHT-LOAD (~1W) REAL PI SHOT.
// Operator-authorized single run (task STAGE6_1MS_LIGHT_LOAD_REAL_PI_SHOT_AND_BENCH_PREPARATION_V1):
//   CNT3/CNT4 = CONNECTED, Vin = 24V, current limit = 0.5A, external load = CC 0.10A
//   or 100ohm/>=5W power resistor (~1W at 10V), no smell/heat/wiring issues.
// Authorized via DSH_CNT34_CONNECTED_CONFIRMED=1 and DSH_LIGHT_LOAD_CONFIRMED=1.
...
// NO automatic advance to 5ms/10ms/12V/continuous closed loop.
// LIGHT_LOAD_1MS_CONTROL_RANGE_INSUFFICIENT: command pinned at 170 kHz and VOUT near
...
// Evidence file names (task STAGE6_1MS_LIGHT_LOAD_REAL_PI_SHOT_AND_BENCH_PREPARATION_V1)
print("EVIDENCE_RAW=evidence/stage6_first_real_pi_shot_real/G9_1MS_LIGHTLOAD_REAL_80E4647A_RAW.txt");
print("EVIDENCE_RESULT=evidence/stage6_first_real_pi_shot_real/G9_1MS_LIGHTLOAD_REAL_80E4647A_RESULT.json");
print("EVIDENCE_REPORT=docs/STAGE6_1MS_LIGHT_LOAD_REAL_PI_REPORT.md");

var OUT="D:\\100W_LLC_F280234_Program\\branch_first_real_pi_shot_v1_1\\evidence\\stage6_first_real_pi_shot_real\\LLC_100W_F28034_BRINGUP_DSH_REAL_SHOT.out";
var MANIFEST="D:\\100W_LLC_F280234_Program\\branch_first_real_pi_shot_v1_1\\evidence\\stage6_first_real_pi_shot_real\\REAL_SHA256SUMS.txt";
...
print("Light-load connected+authorized: "+lightLoad);
if(!lightLoad){ print("ABORT: DSH_LIGHT_LOAD_CONFIRMED != 1. No real shot."); throw "no-light-load-authorization"; }
...
chk("TIMER2_DELTA_59500_62500", t2d>=59500 && t2d<=62500);
  /* PI direction uses shot-local error summary only; never post-IDLE global error. */
...
chk("PIPELINE_FAST_TICKS_51", sfk===51);
chk("PIPELINE_PI_COMPUTE_COUNT_26", spc===26);
chk("PIPELINE_PWM_APPLY_COUNT_26", sac===26);
chk("SHOT_TICK_51", tk===51);
chk("POWER_WRITES_26", pw===26);
...
if (fails===0) { print("STAGE_G_1MS_LIGHTLOAD_REAL_SHOT_PASS"); }
else { print("STAGE_G_1MS_LIGHTLOAD_REAL_SHOT_FAIL"); }
if (smax>=170000 && smv>=(abortRaw-40)) {
  print("LIGHT_LOAD_1MS_CONTROL_RANGE_INSUFFICIENT");
}
print("LIGHTLOAD_SHOT_DONE");

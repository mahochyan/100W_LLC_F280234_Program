@echo off
rem Build the W2_OPEN_LOOP_STEADY binary: W2_OPEN_LOOP_STEADY REAL (PI fully bypassed)
rem Usage: build_flash_open_loop_steady.bat
...
echo === CGT 25.11.1.LTS clean Stage6_OL_STEADY compile (COFF, W2_OPEN_LOOP_STEADY REAL (PI fully bypassed)) ===
...
echo === link (FLASH) ===
"%CGT%\bin\cl2000.exe" --abi=coffabi -v28 -ml -mt -g -O4 --opt_for_speed=5 -ms --diag_warning=225 --diag_wrap=off --display_error_number --gen_func_subsections ^
    -z -m"%BUILD%\LLC_100W_F28034_OPEN_LOOP_STEADY.map" --stack_size=0xBF --warn_sections --entry_point=code_start ^
    -i"%CGT%\lib" -i"%CGT%\include" --reread_libs --diag_wrap=off --display_error_number ^
    --xml_link_info="%BUILD%\LLC_100W_F28034_OPEN_LOOP_STEADY_linkInfo.xml" --rom_model ^
    -o"%BUILD%\LLC_100W_F28034_OPEN_LOOP_STEADY.out" ^
...
echo === BUILD OK (Stage6_OL_STEADY, CGT 25.11.1.LTS, COFF, W2_OPEN_LOOP_STEADY REAL (PI fully bypassed)) ===
endlocal

...
/* True only during the bounded 500us PI window after the 10V handoff: */
 ...
 * conditions that can change during the 500 us window are re-checked per
...
void SHOT_Revoke(Uint16 reason)
{
    ...
    {
        /* E: auto-OST at 500 us. Capture Timer2 BEFORE the planned OST, then
         ...
         * classifies the window as POST_OST, so a normal 500 us timeout cannot
...
void SHOT_FastTask(void)
{
     ...
     * the cage. Expected 10 for the 500 us cage. The cage itself is Timer2
